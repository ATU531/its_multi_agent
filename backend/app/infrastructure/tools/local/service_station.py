...
def get_ip_via_stun():
    ...
    for server in CN_STUN_SERVERS:
        try:
            nat_type, external_ip, external_port = stun.get_ip_info(stun_host=server)
            if external_ip:
                logger.debug(f"[STUN] {server} => {external_ip}")
                return external_ip
        except Exception as e:
            logger.debug(f"[STUN] {server} failed: {e}")
    ...
    try:
        ...
        with urllib.request.urlopen("https://httpbin.org/ip", timeout=5) as resp:
            ...
            if ip:
                logger.debug(f"[HTTP] httpbin.org => {ip}")
                return ip
    except Exception as e:
        logger.debug(f"[HTTP] httpbin.org failed: {e}")
    ...
    try:
        ...
        with urllib.request.urlopen("https://ifconfig.me/ip", timeout=5) as resp:
            ...
            if ip:
                logger.debug(f"[HTTP] ifconfig.me => {ip}")
                return ip
    except Exception as e:
        logger.debug(f"[HTTP] ifconfig.me failed: {e}")
...
async def resolve_user_location_from_text(
    ...
    #  获取 IP (注意：此处目前是获取运行环境 IP，生产环境建议改为前端获取传参注入)
