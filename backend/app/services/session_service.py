...
class SessionService:
    ...
    def load_history(self, user_id: str, session_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"用户 {user_id} 会话 {target_session_id} 文件读取失败 原因 {e}")
    ...
    def save_history(self, user_id: str, session_id: str, chat_history: List[Dict[str, Any]]):
        ...
        try:
            ...
            logger.error(f"保存用户 {user_id} 会话 {target_session_id} 文件失败:{str(e)}")
            return
...
if __name__ == '__main__':
    result1 = session_service.prepare_history(user_id="hzk", session_id="666")
    result1.append({"role": "user", "content": "你好!"})
